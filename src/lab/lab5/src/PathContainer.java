...
public class PathContainer {        // 包含题目[2]-[9]
    ...
    /*@ requires path != null;
      @ assignable \nothing;
      @ ensures \result == (\exists int i; 0 <= i && i < pList.length; pList[i].equals(path));
      @*/
    ...
    /*@ normal_behavior
      ...
      @ ensures (\exists int i; 0 <= i && i < pList.length; pList[i].equals(path) && \result == pidList[i]);
      @ ensures  pList.length == (\old(pList.length) + 1) && pidList.length == (\old(pidList.length) + 1);
    ...
    public int addPath(Path path) {
        if (path != null && path.isValid() && !containsPath(path)) {
            ...
            for (int node : path) {
                //TODO
                globalNodesCount.merge(node, 1, Integer::sum);
            }
            return idCounter++;
        }
        return 0;
