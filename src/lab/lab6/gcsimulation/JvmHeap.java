...
public class JvmHeap extends MyHeap<MyObject> {
    ...
    /*@ public normal_behavior
      ...
      @          (\forall int j; 1 <= j && j <= size; elementData[i].equals(elementData[j]));
      @ 要求：定义后置条件，表示调用此方法前后，elementData中下标大于等于1且小于等于size的各位置元素始终为同一对象引用指向的对象，
      @ 不可使用原子表达式\not_assigned与\not_modified
      @ ensures (\forall int i; 1 <= i && i <= size;
    ...
    public void removeUnreferenced() {
        for (int i = 0; i < getSize(); i++) {
            if (!getYoungestOne().getReferenced()) {
                removeFirst();
            }
        }
    }
    ...
    public /*@pure@*/ MyObject getYoungestOne() {
        ...
        return (MyObject) getElementData()[0];
    }
}