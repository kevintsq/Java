...
public interface IntSet {
    ...
    /*@ assignable ia, a.ia;
      ...
      @ ensures (\forall int i; 0 <= i < ia.length;
                 (\exists int j; 0 <= j < a.ia.length; a.ia[j] == ia[i]));
      @ ensures (\forall int j; 0 <= j < a.ia.length;
                 (\exists int i; 0 <= i < ia.length; ia[i] == a.ia[j]));
      @ */
    ...
    /*@ assignable \nothing;
      ...
      @ requires a != null;
      @ ensures (\forall int i; 0 <= i < \ret.ia.length;
                 ((\exists int j; 0 <= j < ia.length; \ret.ia[i] == ia[j]) &&
                   (\forall int k; 0 <= k < a.ia.length; \ret.ia[i] != a.ia[k])) ||
                 ((\exists int k; 0 <= k < a.ia.length; \ret.ia[i] == a.ia[k]) &&
                   (\forall int j; 0 <= j < ia.length; \ret.ia[i] != ia[j])));
      @ ensures (\forall int j; 0 <= j < ia.length;
                 (\forall int k; 0 <= k < a.ia.length; ia[j] != a.ia[k]) =>
                  (\exists int i; 0 <= i < \ret.ia.length; ia[j] == \ret.ia[i]));
      @ ensures (\forall int k; 0 <= j < a.ia.length;
                 (\forall int j; 0 <= j < ia.length; a.ia[k] != ia[j]) =>
                  (\exists int i; 0 <= i < \ret.ia.length; a.ia[k] == \ret.ia[i]));
      @ also
